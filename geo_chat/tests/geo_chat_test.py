...
class TestGeoData: 
    ...
    def test_subgraph_initialization(self): 
        ...
        assert list(geo.sg.subgraphs.keys())[0] == 'https://api.thegraph.com/subgraphs/name/baiirun/geo'
        ...
        assert data_pd.columns.to_list() == ['entity_name', 'attribute_name', 'string_value']

        # Test that the json data returned is as expected 
        assert isinstance(data_json, str)
        assert data_json == '[{"entity": "Star and crescent", "attribute": "Name", "stringValue": "Star and crescent"}]'

        data_pd = geo.query_triples(first=1, attribute_name='Description', return_type='pd')
        data_json = geo.query_triples(first=1, attribute_name='Description', return_type='json')
        ...
        # Test that the json data returned is as expected 
        assert isinstance(data_json, str)
        assert data_json == '[{"entity": "Law", "attribute": "Description", "stringValue": "A law for individuals to follow"}]'